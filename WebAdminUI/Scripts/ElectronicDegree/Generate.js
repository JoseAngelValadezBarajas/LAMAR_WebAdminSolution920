$(document).ready(function () {
    ...
    $('#PeopleId').blur(function () {
        ...
        setCookie("PeopleCodeId", PeopleCodeId);
        IsValid = true;
        if (PeopleCodeId !== "") {
    ...
    $('#btnValidateStudent').click(function () {
        ...
        if (IsValid) {
            try{
                var alumnoid = $('#PeopleId').val().toString();
            }catch(e){
                console.log(e);
            }
            if (!IsHideStep) {
                        ...
                        if (data.institutionMajors.length > 0) {
                            ...
                            $.each(data.institutionMajors, function (i, item) {
                                ...
                                contentTr = "<tr class='esg-table-body__row' id=" + i + ">" +
                                    ...
                                    "</div>" +
                                    "</div></td >" +
                                    "<td class='esg-table-body__td'><label id='programDesc_" + i + "'>" + item.ProgramDesc + "</label></td >" +
                                    "<td class='esg-table-body__td'>" +
    ...
    $('#btnContinueIM').click(function () {
        ...
        rdoId = rdo.split('_');
        let index = Number(rdoId[1]);
        let labelId = `programDesc_${index}`;
        let labelElement = document.getElementById(labelId);
        var labelText = "";
        if (labelElement) {
            labelText = labelElement.innerText; 
        } else {
            console.error(`No se encontró ningún elemento con el ID '${labelId}'.`);
        }
        if (rdo !== undefined) {
            try {
                $.ajax({
                    url: urlGetAcademicYearsG,
                    dataType: "json",
                    cache: false,
                    type: "GET",
                    data: { peopleId: getCookie("PeopleCodeId"), institutionName: labelText},
                    success: function (data) {
                        setSessionCookie("academicInfo", data);
                        try{
                            var startdate = convertToReadableDate(data[1].Date);
                            var enddate = convertToReadableDate(data[0].Date);
        ...
        try{
            const storedJsonData = getSessionCookie("academicInfo");
            $('#StartDate').val(storedJsonData[1].Date);
            $('#EndDate').val(storedJsonData[0].Date);
        }catch(e){
            console.log(e);
        }
    });
    ...
    $('#btnValidateIM').click(function () {
        ...
        try{
            var valorInput = $('#MajorName').val().trim(); 
            var palabras = valorInput.split(' ');
            if (palabras.length > 0 && (palabras[0] === "MAESTRÍA" || palabras[0] === "MAESTRIA" || palabras[0] === "MA.")) {
                setCookie("BackStudies", "2");
            } else {
                setCookie("BackStudies", "4"); 
            }
        }catch(e){
            console.log(e);
        }
        setCookie("InstitutionCode", $('#MajorName').val());    
        if (IsValid) {
            $('#StartDate').attr('readonly', true).attr('disabled', true);
            $('#EndDate').attr('readonly', true).attr('disabled', true);
            $('#StartDate').parent().removeClass('has-error');
    ...
    $('#btnContinueBS').click(function () {
        ...
        if (rdoId !== undefined) {
            ...
            $('#StateCatalogBS').val(backgroundStudies[Number(rdoId[1])].StateCode);
             //mod addv 13082024
            LevelStudy = backgroundStudies[Number(rdoId[1])].LevelStudy;
        ...
        document.getElementById("StateCatalogBS").value = "14";
        document.getElementById("StateCatalogBS").disabled = true;
        document.getElementById("StateBS").value = "Jalisco";
        document.getElementById("StateBS").textContent = "Jalisco";
        document.getElementById("TypeBackgroundStudyCatalog").value = getCookie("BackStudies");

        //mod addv 13082024
    
        document.getElementById("TypeBackgroundStudyCatalog").value = LevelStudy;
