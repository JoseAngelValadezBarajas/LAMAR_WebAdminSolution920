$(document).ready(function () {
    ...
    $('.esg-form__input').on('input', function() {
        var $table = $("#records_table tbody");
        var cols = [];
        var i = 0;
        var indexInstitution = $('input#hdnIndexInstitution').val();
        for (var i = 0; i < indexInstitution; i++) {
            if($("#InsSep_" + i).val()!= ''
                && ($("#CampusSepCode_" + i).val() != ''
                    && $("#ResponsibleId_" + i).val() > 0)){
                $("#FederalEntityId_" + i).val(14);
                $("#FederalEntityId_" + i).val("14");
                $("#FederalEntityId_" + i).prop("disabled", true);
            }
        };
    });
...
document.addEventListener("DOMContentLoaded", function() {
        const elements = document.querySelectorAll('.data_campus');
        let dataIdValues = [];
        elements.forEach(function(element) {
            let dataIdValue = element.getAttribute('data-id');
            dataIdValues.push(dataIdValue);  
        });
        try {
            for (var i = 0; i < dataIdValues.length; i++) {
                (function (i) {
                    $.ajax({
                        url: "http://localhost/PowerCampusWebAdministrationServices_LAMAR/api/people/yts/getcampusrvoe?campusRvoe=" + dataIdValues[i],
                        dataType: "json",
                        cache: false,
                        type: "GET",
                        success: function (data) {
                            for (var j = 0; j < data.length; j++) {
                                var campusSepCode = data[j].AgreementNumber;
                                var idActual = i;
                                var selectElement = document.getElementById("CampusSepCode_" + idActual);
                                if (selectElement) {
                                    var option = document.createElement("option");
                                    option.value = campusSepCode;
                                    option.text = campusSepCode;
