...
$(document).ready(function () {
        ...
        if (StudiesProgramIsValid) {
            ...
            $.ajax({
                ...
                if (data && data.coursesViewModel) {
                    ...
                    //mod addv 22082024
                    const roundAverageAddv = Number(average).toFixed(2) == 10.00 || Number(average).toFixed(2) == 5.00 ? Number(coursesViewModel.Average).toFixed(0) : Number(coursesViewModel.Average).toFixed(2);
                    let averageString_Addv = roundAverageAddv.toString();


                    $('#TotalCourses').val(coursesViewModel.TotalCourses);
                    $('#TotalCredits').val((coursesViewModel.TotalCredits).toFixed(2));
                    $('#AsignedCourses').val(coursesViewModel.AsignedCourses);
                    //$('#Average').val(coursesViewModel.Average);
                    //mod addv 22082024
                    $('#Average').val(averageString_Addv);
                    ...
                    $.each(coursesViewModel.CoursesDetailViewModel, function (i, item) {
                        ...
                        averageString = averageString.slice(0, (averageString.indexOf(".")) + 3);
                        //mod addv 22082024
                        const roundFinalGrade = Number(average).toFixed(2) == 10.00 || Number(item.FinalGrade).toFixed(2) == 5.00 ? item.FinalGrade.toFixed(0) : item.FinalGrade.toFixed(2);
                    ...
                    //console.log(coursesViewModel.TotalCourses);
                    //console.log(coursesViewModel.AsignedCourses);
                    //if(coursesViewModel.TotalCourses >= coursesViewModel.AsignedCourses){
                    //    document.cookie = "coursesViewModel=0";
                    //}else{
                    //    document.cookie = "coursesViewModel=1";
                    //}
                }
    ...
    $(document).on("click", "#btnApplyAddCourse", function () {
        $.each(coursesToAdd, function (i, item) {
            ...
            averageString = averageString.slice(0, (averageString.indexOf(".")) + 3);
            //mod addv 22082024
            const finalGrade = Number(item.FinalGrade).toFixed(2) == 10.00 || Number(item.FinalGrade).toFixed(2) == 5.00 ? item.FinalGrade.toFixed(0) : item.FinalGrade.toFixed(2);
            ...
            courses.push({
                ...
                IsInclude: item.IsInclude,
                Credit: Number(item.Credit),
                FinalGrade: Number(item.FinalGrade).toFixed(2) == 10.00 || Number(item.FinalGrade).toFixed(2) != 5.00 ? item.FinalGrade.toFixed(0) : item.FinalGrade.toFixed(2),
                CourseCycle: item.CourseCycle,
                EventName: item.CourseName,
                SepId: item.SepId,
                TranscriptDetailCertificateId: item.TranscriptDetailCertificateId,
...
// #region dates
function oneDayBeforeDate() {
    let fechaHabilAnterior = new Date();

    // Obtener el día de la semana actual (0 para domingo, 1 para lunes, etc.)
    let diaDeLaSemana = fechaHabilAnterior.getDay();
    
    // Si hoy es lunes (1), restamos 3 días para volver al viernes. Si no, solo restamos 1 día.
    if (diaDeLaSemana === 1) {
        fechaHabilAnterior.setDate(fechaHabilAnterior.getDate() - 3);
    } else {
        fechaHabilAnterior.setDate(fechaHabilAnterior.getDate() - 1);
    }

    // Formatear la fecha a YYYY-MM-DD
    let year = fechaHabilAnterior.getFullYear();
    let month = ('0' + (fechaHabilAnterior.getMonth() + 1)).slice(-2); // getMonth() devuelve 0 para enero, 11 para diciembre
    let day = ('0' + fechaHabilAnterior.getDate()).slice(-2);
...
function calculateCredits() {
    ...
    //mod addv 22082024
    const roundAverageAddv1 = Number(average).toFixed(2) == 10.00 || Number(average).toFixed(2) == 5.00 ? Number(average).toFixed(0) : Number(average).toFixed(2);
    let averageString_Addv1 = roundAverageAddv1.toString();

    $('#AsignedCourses').val(totalAssigned);
    $('#ObtainedCredits').val((obtainedCredits).toFixed(2));
    //$('#Average').val(average);
     //mod addv 22082024
    $('#Average').val(averageString_Addv1);
}
...
// #region Issuing Certificate
/*function updateCourseCertificateOption() {
    let cookieValue = document.cookie.replace(/(?:(?:^|.*;\s*)coursesViewModel\s*\=\s*([^;]*).*$)|^.*$/, "$1");
    console.log(cookieValue);
    if(cookieValue==true){
        document.getElementById("CertificationType").value = 79;
        document.getElementById("CertificationType").value = "79";
    }else{ 
        document.getElementById("CertificationType").value = 80;
        document.getElementById("CertificationType").value = "80";
    }
    document.getElementById("CertificationType").disabled = true;
}*/
    ...
    $.ajax({
        ...
        .done(function (response) {
            ...
            $('#IssuingPlace').html(`
                    ...
                    ${issuingPlaceHtml}
                `);
            document.getElementById("IssuingPlace").value = "14";
            document.getElementById("IssuingPlace").disabled = true;
            let cookieValue = document.cookie.replace(/(?:(?:^|.*;\s*)coursesViewModel\s*\=\s*([^;]*).*$)|^.*$/, "$1");
            //MOD 10072024
...
function updatePreviewData() {
    ...
    if (Number($('#Average').val()) == 10.00 || Number($('#Average').val()) == 5.00) {
        PreviewData.gpa = Number($('#Average').val()).toFixed(0);
    ...
    courses.map((course) => {
        if (course.IsInclude) {
            PreviewData.courses.push({
                ...
                //mod addv 22082024
                finalGrade: Number(course.FinalGrade).toFixed(2) == 10.00 || Number(course.FinalGrade).toFixed(2) == 5.00 ? Number(course.FinalGrade).toFixed(0) : Number(course.FinalGrade).toFixed(2),
            ...
            //Mod addv 22082024
            var FinalGrade1 = Number(course.FinalGrade).toFixed(2) == 10.00 || Number(course.FinalGrade).toFixed(2) == 5.00 ? Number(course.FinalGrade).toFixed(0) : Number(course.FinalGrade).toFixed(2);
...
window.onload = function() {
    let issuingDateInput = document.querySelector('#divIssuingDate .esg-form__input-group #IssuingDate');
    if (issuingDateInput) {
        issuingDateInput.value = oneDayBeforeDate(); 
        issuingDateInput.readOnly = true;
    }
};